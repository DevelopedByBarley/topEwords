...
const FORM_POS_FIELDS: Record<FormPos, Array<keyof WordFormData>> = {
    verb: ['form_base', 'verb_past', 'verb_past_participle', 'verb_present_participle', 'verb_third_person'],
    noun: ['noun_plural'],
...
export default function WordFormFields({ form, onChange, errors = {}, autoFocus = false, afterWordSlot }: WordFormFieldsProps) {
    ...
    const primaryPos = (FORM_POS as readonly string[]).includes(form.part_of_speech) ? (form.part_of_speech as FormPos) : null;
    ...
    const renderBlock = (pos: FormPos): ReactNode => {
        if (pos === 'verb') {
            return (
                <div key="verb" className="rounded-xl border bg-muted/30 px-4 py-4 flex flex-col gap-3">
        ...
        if (pos === 'noun') {
            return (
                <div key="noun" className="rounded-xl border bg-muted/30 px-4 py-4 flex flex-col gap-3">
        ...
        return (
            <div key="adj" className="rounded-xl border bg-muted/30 px-4 py-4 flex flex-col gap-3">
        ...
        <>
            ...
            <div className="flex flex-col gap-3">
                <button
                    ...
                    className="flex items-center gap-1 self-start text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
                >
                    <ChevronDown className={`size-3.5 transition-transform ${showOther ? '' : '-rotate-90'}`} />
