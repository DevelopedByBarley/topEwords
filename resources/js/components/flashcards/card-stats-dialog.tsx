    ...
    return (
        <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
            <StatsRow label="Állapot">
                <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${stateBadgeClass(r.state)}`}
                >
            ...
            {r.due_at && (
                <StatsRow label="Esedékesség">
                    <span className="font-medium">
                        {formatDue(r.due_at, r.state, now)}
                    </span>
                </StatsRow>
            )}
            {r.introduced_on && (
...
export default function CardStatsDialog({
    card,
    onClose,
    now,
}: {
            ...
            <DialogContent className="flex max-h-[85dvh] w-[calc(100vw-2rem)] flex-col sm:max-w-sm">
                <DialogHeader className="pr-8">
                    ...
                    <DialogDescription className="sr-only">
                        A kártya SRS-adatai irányonként: állapot, intervallum,
                        ismétlések és tévesztések.
                    </DialogDescription>
                </DialogHeader>
                                ...
                                {r.previous_state && (
                                    <div className="mt-3">
                                        ...
                                        <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
                                            <StatsRow label="Állapot">
                                                <span
                                                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${stateBadgeClass(r.previous_state.state)}`}
                                                >
                                                    {stateLabelFull(
