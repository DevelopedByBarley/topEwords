...
import { Activity, CheckCheck, Clock, BookMarked, Mic, Sparkles, Trash2, Users, TrendingUp, Flame } from 'lucide-react';
...
interface Props {
    ...
    aiAccessUsers: AiUser[];
}

export default function AdminIndex({ stats, topStreaks, recentUsers, mostActive, registrationsByDay, aiAccessUsers }: Props) {
    const aiForm = useForm({ email: '' });
                    ...
                    <div>
                        ...
                        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 space-y-4">
                            <form
                                onSubmit={(e) => {
                                    e.preventDefault();
                                    aiForm.post(toggleAiAccess().url, { onSuccess: () => aiForm.reset() });
                                }}
                            ...
                            >
                                <Input
                                    ...
                                    value={aiForm.data.email}
                                    onChange={(e) => aiForm.setData('email', e.target.value)}
                                    className="bg-zinc-800 border-zinc-700 text-zinc-100 placeholder:text-zinc-500"
                                />
                                <Button type="submit" size="sm" disabled={aiForm.processing}>
                                    <Sparkles className="size-3.5 mr-1.5" />
                                    Toggle
                                </Button>
                            </form>
                            {aiForm.errors.email && (
                                <p className="text-xs text-red-400">{aiForm.errors.email}</p>
                            )}
                            {aiAccessUsers.length > 0 && (
                                <div className="space-y-2">
                                    <p className="text-xs text-zinc-500 uppercase tracking-wider">Manuálisan jogosult felhasználók</p>
                                    {aiAccessUsers.map((u) => (
                                        <div key={u.id} className="flex items-center justify-between rounded-lg border border-zinc-800 px-3 py-2">
                                            <div>
                                                <div className="text-sm font-medium">{u.name}</div>
                                                <div className="text-xs text-zinc-500">{u.email}</div>
                                            </div>
                                            <button
                                                onClick={() => router.post(toggleAiAccess().url, { email: u.email })}
