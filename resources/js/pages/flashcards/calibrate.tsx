...
import {
    rate as rateCard,
    skip as skipCalibration,
} from '@/routes/flashcards/calibrate';
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            ...
            <div className="flex items-center gap-2">
                <input
                    ...
                    className="w-16 rounded-md border bg-background px-2 py-1 text-center text-base md:text-sm"
                />
...
export default function FlashcardCalibrate({
    deck,
    cards,
    totalRemaining,
    calibIntervals,
    ...
    const ratings = [
        ...
        },
        {
            rating: 2 as const,
            label: 'Valamennyire',
            description: `≈ ${intervals.somewhat_min}–${intervals.somewhat_max} nap`,
            shortcut: '2',
            className:
        ...
        },
        {
            rating: 3 as const,
            label: 'Tudom',
            description: `≈ ${intervals.know_min}–${intervals.know_max} nap`,
            shortcut: '3',
            className:
        ...
        },
        {
            rating: 4 as const,
            label: 'Jól tudom',
            description: `≈ ${intervals.well_min}–${intervals.well_max} nap`,
            shortcut: '4',
            className:
                'border-blue-500/50 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950',
        },
    ];
    ...
    const handleRate = useCallback(
        ...
        [current, currentIndex, cards.length, deck.id, showBack, intervals],
    );
                ...
                <div className="flex min-h-[80vh] flex-col items-center justify-center px-4">
                    <div className="w-full max-w-md">
                        ...
                        <p className="mb-6 text-sm text-muted-foreground">
                            Állítsd be, hogy az egyes értékelések mekkora
                            ismétlési intervallumot jelentsenek. A beállítások
                            mentésre kerülnek a következő kalibráláshoz.
                        </p>

                        <div className="mb-6 flex flex-col gap-4 rounded-xl border bg-card p-5">
                            <IntervalInput
                                label="Valamennyire"
                            ...
                            <IntervalInput
                                ...
                                onChangeMin={(v) =>
                                    setIntervals((p) => ({ ...p, know_min: v }))
                                }
                                onChangeMax={(v) =>
                                    setIntervals((p) => ({ ...p, know_max: v }))
                                }
                            />
                            <IntervalInput
                                label="Jól tudom"
                                ...
                                onChangeMin={(v) =>
                                    setIntervals((p) => ({ ...p, well_min: v }))
                                }
                                onChangeMax={(v) =>
                                    setIntervals((p) => ({ ...p, well_max: v }))
                                }
                            />
                        </div>
                ...
                <div className="flex min-h-[80vh] flex-col items-center justify-center gap-6 px-4 text-center">
                    ...
                    {totalRemaining > cards.length && (
                        <p className="text-sm text-muted-foreground">
                            Még <strong>{totalRemaining - cards.length}</strong>{' '}
                            kártya vár kalibrálásra.
                        </p>
                    )}
            ...
            <div className="mx-auto flex min-h-[80vh] max-w-2xl flex-col px-4 py-6">
                ...
                {showSettings && (
                    <div className="mb-6 flex flex-col gap-3 rounded-xl border bg-card p-4">
                        <p className="text-xs text-muted-foreground">
                            Az intervallumok a következő értékeléstől érvényesek
                            és mentésre kerülnek.
                        </p>
                        <IntervalInput
                            label="Valamennyire"
                            min={intervals.somewhat_min}
                            max={intervals.somewhat_max}
                            onChangeMin={(v) =>
                                setIntervals((p) => ({ ...p, somewhat_min: v }))
                            }
                            onChangeMax={(v) =>
                                setIntervals((p) => ({ ...p, somewhat_max: v }))
                            }
                        />
                        <IntervalInput
                            label="Tudom"
                ...
                <div
                    ...
                    style={
                        current.color
                            ? { borderColor: current.color, borderWidth: 2 }
                            : {}
                    }
                >
                    ...
                    <div className="p-6 text-center">
                        ...
                        {current.calibration_direction === 'front_to_back' &&
                            current.front_notes && (
                                <RichTextContent
                                    html={current.front_notes}
                                    className="mt-2 text-sm text-muted-foreground"
                                />
                            )}
                        {current.calibration_direction === 'back_to_front' &&
                            current.back_notes && (
                                <RichTextContent
                                    html={current.back_notes}
                                    className="mt-2 text-sm text-muted-foreground"
                                />
                            )}
                    </div>
                    ...
                    <>
                            ...
                            {ratings.map(
                                ({
                                    ...
                                    <button
                                        ...
                                        className={`flex flex-col items-center justify-center gap-1 rounded-xl border-2 px-2 py-4 text-sm font-medium transition-colors ${className}`}
                                    >
                        ...
                        <p className="mt-4 text-center text-xs text-muted-foreground">
                            ...
                            {['1', '2', '3', '4'].map((k) => (
                                <kbd
                                    key={k}
                                    className="mx-0.5 rounded border px-1.5 py-0.5 text-xs"
                                >
                                    {k}
                                </kbd>
                            ))}
