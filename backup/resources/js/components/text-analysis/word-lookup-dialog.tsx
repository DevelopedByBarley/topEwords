import { BookMarked, CheckCheck, Clock, Loader2, Mic, Plus, Sparkles, Volume2 } from 'lucide-react';
import { useEffect, useState } from 'react';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogTitle } from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { EMPTY_CUSTOM_WORD_FORM, getCsrfToken, POS_LABELS } from '@/components/text-analysis/types';
import type { CustomWordForm, LookupResult, TokenStatus } from '@/components/text-analysis/types';
import { status as customWordStatus, store as customWordsStore } from '@/routes/custom-words';
...
interface WordLookupDialogProps {
    ...
    onStatusChange: (word: string, prevStatus: string | null, nextStatus: TokenStatus | null, fallback: TokenStatus) => void;
    onCustomAdded: (word: string) => void;
}

const STATUS_BUTTONS = [
    { s: 'known', label: 'Tudom', Icon: CheckCheck, active: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400', hover: 'hover:bg-green-50 hover:text-green-700' },
    { s: 'learning', label: 'Tanulom', Icon: Clock, active: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400', hover: 'hover:bg-blue-50 hover:text-blue-700' },
    { s: 'saved', label: 'Később', Icon: BookMarked, active: 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-400', hover: 'hover:bg-orange-50 hover:text-orange-700' },
    { s: 'pronunciation', label: 'Kiejtés', Icon: Mic, active: 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-400', hover: 'hover:bg-violet-50 hover:text-violet-700' },
] as const;

export default function WordLookupDialog({ word, context, hasAiAccess, onClose, onStatusChange, onCustomAdded }: WordLookupDialogProps) {
    ...
    const handleStatusClick = async (newStatus: string) => {
        ...
        const path = lookupResult.type === 'word'
            ? wordStatus.url(lookupResult.id)
            : customWordStatus.url(lookupResult.id);
        const nextStatus = lookupStatus === newStatus ? null : (newStatus as TokenStatus);
        const prevStatus = lookupStatus;

        setLookupStatus(nextStatus);
        onStatusChange(word, prevStatus, nextStatus, lookupResult.type === 'word' ? 'in_list' : 'not_in_list');
    ...
    const handleGeminiAutofill = async () => {
        ...
        try {
            ...
            const res = await fetch(geminiLookup.url({ query }), {
            ...
            if (data.is_real_word === false) {
                setGeminiNotice(data.message ?? 'Ez nem tűnik valódi angol szónak.');
                return;
            }
    ...
    return (
        <Dialog open={word !== null} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className={`w-[calc(100vw-2rem)] gap-0 overflow-hidden p-0 ${lookupResult?.type === 'not_found' ? 'sm:max-w-lg' : 'sm:max-w-md'}`}>
                {lookupLoading && (
                                    ...
                                    {lookupResult.type !== 'not_found' && (
                                        <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
                                            ...
                                            {lookupResult.part_of_speech && (
                                                <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                                                    {POS_LABELS[lookupResult.part_of_speech] ?? lookupResult.part_of_speech}
                                                </span>
                                            )}
                                        </div>
                                    )}
                            ...
                            {lookupResult.type !== 'not_found' && (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {STATUS_BUTTONS.map(({ s, label, Icon, active, hover }) => (
                                            <button
                                                key={s}
                                                    ...
                                                    <div className="space-y-2 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-950/30">
                                                        {context && (
                                                            <p className="text-xs text-blue-700 dark:text-blue-300">
                                                                ...
                                                                {context.replace(new RegExp(`\\b${lookupResult.word}\\b`, 'gi'), (m) => `「${m}」`)}
                                                            </p>
                                                        )}
                                                        {contextExplanation && (
                                                        ...
                                                        {!contextExplanation && context && (
                                                            <p className="text-xs italic text-blue-500 dark:text-blue-400">Nyomj Gemini AI-ra a kontextuális magyarázathoz.</p>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
