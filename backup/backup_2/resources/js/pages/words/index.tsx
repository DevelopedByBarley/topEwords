...
import type {CustomWord, Word, WordFormData, WordStatus} from '@/components/words/types';
...
export default function WordsIndex({
    ...
    const [customWordForm, setCustomWordForm] = useState<WordFormData>(EMPTY_WORD_FORM);
    ...
    const [editCustomWordForm, setEditCustomWordForm] = useState<WordFormData>(EMPTY_WORD_FORM);
    const [editWordId, setEditWordId] = useState<number | null>(null);
    const [editWordForm, setEditWordForm] = useState<WordFormData>(EMPTY_WORD_FORM);
    ...
    const [practiceModalWord, setPracticeModalWord] = useState<PracticeWord | null>(null);
...
return;
    ...
    function handleCustomWordStatus(wordId: number, newStatus: Exclude<WordStatus, null>) {
    ...
    return (
        <>
            ...
            {/* Practice modal */}
            <PracticeModal key={practiceModalWord?.word ?? 'practice'} word={practiceModalWord} onClose={() => setPracticeModalWord(null)} />
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 pb-24 md:px-6 md:pt-6 md:pb-28">
                ...
                <div className="relative overflow-hidden rounded-3xl bg-primary p-6 md:p-8">
                    ...
                    <div className="relative flex items-start justify-between gap-3">
                        ...
                        >
                            ...
                            {customStats.total > 0 && (
                                <span className="text-xs font-normal opacity-70">{customStats.total}</span>
                            )}
                        </button>
                    </div>
                ...
                <Dialog open={showAddCustomWord} onOpenChange={(open) => {
 if (!open) {
 setShowAddCustomWord(false); setCustomWordErrors({}); setCustomWordForm(EMPTY_WORD_FORM); 
} 
}}>
 ...
 setShowAddCustomWord(false); setCustomWordErrors({}); setCustomWordForm(EMPTY_WORD_FORM); 
                ...
                <div className="flex flex-col gap-4 rounded-3xl bg-card p-4 shadow-sm md:p-5">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Select value={String(filters.per_page)} onValueChange={(v) => navigate({ per_page: Number(v), page: 1 })}>
                            <SelectTrigger className="w-28 rounded-full border-0 bg-muted text-sm">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                                {[20, 50, 100, 200, 300, 400, 500, 1000].map((n) => (
                                    <SelectItem key={n} value={String(n)}>{n} / oldal</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    {!search && (
                        <FilterGroup label="Szint">
                            <FilterChip active={filters.level === null} onClick={() => handleLevelClick(null)}>
                                Mind
                            </FilterChip>
                            {LEVELS.map((l) => (
                                <FilterChip
                    ...
                    <FilterGroup label="Státusz">
                        <FilterChip active={!filters.status} onClick={() => handleStatusFilter('')}>
                            Mind
                        </FilterChip>
                        {STATUS_CONFIG.map(({ value, label, icon: Icon, filterActive }) => (
                            <FilterChip
                                key={value}
                            ...
                            >
                                ...
                                {label}
                                <span className="font-normal opacity-75">{stats[value].toLocaleString()}</span>
                            </FilterChip>
                        ))}
                    </FilterGroup>

                    <FilterGroup label="Fontosság">
                        <FilterChip active={filters.importance === null} onClick={() => handleImportanceFilter(null)}>
                            Mind
                        </FilterChip>
                        {[1, 2, 3, 4, 5].map((n) => (
                            <FilterChip
                    ...
                    {folders.length > 0 && (
                        <FilterGroup label="Mappa">
                            <FilterChip active={filters.folder === null} onClick={() => handleFolderFilter(null)}>
                                Mind
                            </FilterChip>
                            {folders.map((f) => (
                                <FilterChip
                                ...
                                >
                                    ...
                                    {f.name}
                                    <span className="font-normal opacity-75">{f.words_count}</span>
                                </FilterChip>
                            ))}
                        </FilterGroup>
                    )}
                    ...
                    {customStats.total > 0 && (
                        <FilterGroup label="Forrás">
                            <FilterChip active={customFilter === 'all'} onClick={() => setCustomFilter('all')}>
                                Minden szó
                            </FilterChip>
                            <FilterChip active={customFilter === 'custom'} onClick={() => setCustomFilter('custom')}>
                                <Plus className="size-3.5" />
                                Saját szavak
                                <span className="font-normal opacity-75">{customStats.total}</span>
                            </FilterChip>
                        </FilterGroup>
                    )}
                    ...
                    {!search && (
                        <FilterGroup label="Betű">
                            <FilterChip
                                active={!filters.letter || filters.letter === 'ALL'}
                            ...
                            {LETTERS.map((letter) => {
                                ...
                                return (
                                    <FilterChip key={letter} active={isActive} onClick={() => handleLetterClick(letter)}>
                                        {letter}
                                        {hasMarks && !isActive && (
                    ...
                    <div className="overflow-hidden rounded-3xl bg-card shadow-sm">
                        <ul className="divide-y">
                            {unifiedList.map((item) =>
                                <li
                                    key={item.type === 'custom' ? `custom-${item.data.id}` : item.data.id}
                                    className={`flex items-center gap-3 px-3 py-2.5 transition-colors ${statusRowBg(item.data.status)}`}
                                >
                                    <button
                                        onClick={() => (item.type === 'custom' ? setSelectedCustomWordId(item.data.id) : setSelectedWordId(item.data.id))}
                                    ...
                                    <StatusButtons
                                        ...
                                        onSelect={(s) =>
                                            item.type === 'custom'
                                                ? handleCustomWordStatus(item.data.id, s)
...
navigate({ page: pageNum });
            ...
            <Dialog open={selectedCustomWordId !== null} onOpenChange={(open) => {
 if (!open) {
...
return null;
                        ...
                        return (
                            <>
                                <div className="border-b bg-linear-to-br from-primary/8 to-primary/3 px-6 pb-4 pt-5">
                                ...
                                <div className="max-h-[60vh] space-y-4 overflow-y-auto px-6 py-5">
                                    ...
                                    <StatusButtons
                                        variant="modal"
                                        current={cw.status}
                                        onSelect={(s) => handleCustomWordStatus(cw.id, s)}
                                    />
                                    <ImportanceStars
                                        value={cw.importance}
                                        onChange={(v) => handleCustomWordImportance(cw.id, v)}
                                    />
...
return;
                                    ...
                                    {isAdmin && (
                                        <div className="flex flex-col gap-3 border-t pt-4">
                                            <WordInsightPanel key={cw.word} word={cw.word} />
                                        </div>
                                    )}
                                    ...
                                    <div className="flex gap-2 border-t pt-4">
                                        <Button
                                            ...
                                            onClick={() => {
                                                ...
                                                setEditCustomWordForm(wordToFormData(cw));
                                            }}
                                        >
            ...
            <Dialog open={editCustomWordId !== null} onOpenChange={(open) => {
 if (!open) {
setEditCustomWordId(null);
} 
}}>
                <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-lg">
                    ...
                    <div className="max-h-[65vh] overflow-y-auto px-6 py-5 flex flex-col gap-4">
                        <WordFormFields form={editCustomWordForm} onChange={setEditCustomWordForm} autoFocus />
                    </div>
                    <div className="flex gap-2 border-t px-6 py-4">
...
setSelectedWordId(null);
                        ...
                        <>
                            ...
                            <div className="max-h-[65vh] space-y-4 overflow-y-auto px-6 py-5">
                                ...
                                <StatusButtons
                                    ...
                                    onSelect={(s) => handleStatus(selectedWord, s)}
                                />

                                {/* Fontosság */}
                                <ImportanceStars
                                    value={selectedWord.importance}
                                    onChange={(v) => handleImportance(selectedWord.id, v)}
                                />
                                ...
                                {isAdmin && (
                                    <div className="border-t pt-4 flex flex-col gap-3">
                                        <WordInsightPanel key={selectedWord.word} word={selectedWord.word} />
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                ...
                                                setEditWordForm(wordToFormData(selectedWord));
                                            }}
                                        >
                                            <Pencil className="size-3.5" />
                                            Admin szerkesztés
            ...
            {isAdmin && (
                <Dialog open={editWordId !== null} onOpenChange={(open) => {
 if (!open) {
setEditWordId(null);
} 
}}>
                    <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-lg">
                        ...
                        <div className="max-h-[65vh] overflow-y-auto px-6 py-5 flex flex-col gap-4">
                            <WordFormFields form={editWordForm} onChange={setEditWordForm} autoFocus />
                        </div>
                        <div className="flex gap-2 border-t px-6 py-4">
