            ...
            <div className="px-6 py-10">
                <div className="mb-8">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        Egészítsd ki a mondatot a hiányzó szóval – aktiváld a
                        passzív szókincsedet!
                    </p>
                </div>
                            ...
                            {Object.entries(STATUS_LABELS).map(
                                ([value, label]) => (
                                    <button
                                        ...
                                        className={`rounded-lg border px-4 py-2.5 text-left text-sm font-medium transition-colors ${status === value ? 'border-primary bg-primary/5 text-primary' : 'bg-background hover:bg-muted'}`}
                                    >
                                        {label}
                                    </button>
                                ),
                            ...
                            {Object.entries(DIFFICULTY_LABELS).map(
                                ([value, label]) => (
                                    <button
                                        ...
                                        className={`rounded-lg border px-4 py-2.5 text-left text-sm font-medium transition-colors ${difficulty === value ? 'border-primary bg-primary/5 text-primary' : 'bg-background hover:bg-muted'}`}
                                    >
                                        {label}
                                    </button>
                                ),
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {[10, 20, 50].map((n) => (
                                <button
                                    ...
                                    className={`rounded-lg border px-4 py-2.5 text-center text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${count === n ? 'border-primary bg-primary/5 text-primary' : 'bg-background hover:bg-muted'}`}
                                >
                                    {n}
                                </button>
                            ))}
                            {available <= 100 && (
                                <button
                                    ...
                                    className={`col-span-2 rounded-lg border px-4 py-2.5 text-center text-sm font-medium transition-colors ${count === 500 ? 'border-primary bg-primary/5 text-primary' : 'bg-background hover:bg-muted'}`}
                                >
                                    Összes ({available})
                                </button>
                            )}
                                ...
                                filteredWords.map((w) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {w.is_custom && (
                                                <span className="shrink-0 rounded bg-violet-100 px-1.5 py-0.5 text-xs font-medium text-violet-700 dark:bg-violet-900/30 dark:text-violet-300">
                                                    saját
                                                </span>
                                            )}
                                        </button>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {answerState === 'wrong' && (
                        <div className="rounded-xl border bg-muted/40 px-4 py-3 text-sm">
                            ...
                            <span className="ml-3 text-muted-foreground italic">
                                "{card.sentence.replace('_____', card.answer)}"
                            </span>
                        </div>
                    )}
                    ...
                    {usedBaseForm && (
                        <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm dark:border-green-800 dark:bg-green-950/20">
                            ...
                            <span className="ml-2 text-green-700 italic dark:text-green-400">
                                "{card.sentence.replace('_____', card.answer)}"
                            </span>
                        </div>
                    )}
